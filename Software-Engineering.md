...
UML 利用图形化机制描述系统的分析和设计模型，定义了四类十余种视图：
...
**UML 交互图（interactive diagram）**显示一个交互过程，包括一组对象和它们间的关系，以及对象间传递的消息。交互图主要包括顺序图和通信图，顺序图强调交互和时间的关系，通信图则没有反应时间信息。

> UML 交互图属于动态模型，用于对基本行为（功能）进行建模。
...
**UML 状态图（state diagram）**是描述状态机的一种工具。

> UML 状态图属于动态模型，用于对高级行为（功能）进行建模。
...
| 动作和活动 | ![image-20220407213822129](.Software-Engineering-assets/image-20220407213822129.png) | 单个或一组动作（活动）写在转移箭头上方，动作也被称为事件或守卫条件。 |
|            |                                                              |                                                              |
