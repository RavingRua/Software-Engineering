...
+ **构件**：软件中具有某种功能的可复用、可替代、已实现模块，表示系统中的计算过程或数据结构；
+ **连接件**：建立构件关联和信息传递的途径；
+ **约束**：构件交互和关联时必须满足的限定条件。
...
这些架构的实现为所有利益相关者提供了交流社区，并简化了软件工作流中的各个流程，但也因此对整个软件行业产生了深远的影响。为了确保这些架构以及架构之上的其他架构能够始终有效，必须关注架构设计。
...
| 模块组成 | 项目经理，架构师 | 架构模型 | UML，其他体系结构图                            | 分配项目资源和人员分工，考虑构件和架构重用 |
...
+ 数据流架构：该架构下包含一组相对独立的过程，约束则是这些过程必须按特定顺序执行，且一个或一组过程的输出将成为下一个或下一组过程的输入，任意环节出错将导致整个架构停止。这些过程构件被称为“过滤器”，通过“管道”连接。典型的实现有 Shell 批处理作业脚本、编译器、媒体播放器等；
  ...
  ![image-20220426214302976](assets/Software-Engineering/image-20220426214302976.png)
...
+ 调用和返回架构：这类架构在各级抽象中都特别常见，包含主程序构件和其他子程序构件，这些构件组成一种树状调用关系，这种约束同样可以防止错误扩散，并让程序结构变得比较清晰。根据各个构件间的调用方式还可分为本地过程调用和远程过程调用，典型的实现有类 C 程序设计语言结构；
  ...
  ![image-20220426214856325](assets/Software-Engineering/image-20220426214856325.png)
...
**架构模式**是相比架构风格更加细化的构件关系描述，用于解决在特定上下文和约束下的常见应用问题。“四人帮”在《设计模式》一书中是这么描述面向对象范式中的模式的：
...
![E-R](assets/Software-Engineering/E-R.png)
