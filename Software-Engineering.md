...
4. 必要时进一步细化用例（<a href="#4-3">UML 交互图</a>和<a href="#4-4"> UML 活动图</a>）
...
>
> 根据实践经验，小型项目在每层用例细化模型中，一般最好使用不超过 20 个用例，保证建模粒度较细以接近实现。
...
**流程图（flow chart）**是用于描述系统流程的工具，通常包含一组符号和文字说明。流程图类似于 UML 标准中的<a href="#4-4">活动图</a>。
...
第四部分主要介绍一些软件工程中常用 UML 技术，符号图片主要来自 [Microsoft Visio](https://www.microsoft.com/en-us/microsoft-365/visio/flowchart-software) 。UML 将建模分为静态和动态建模。
...
| 扩展   | ![image-20220331183340790](.Software-Engineering-assets/image-20220331183340790.png) | 虚线箭头附加构造型，表示一个用例可能在另一个用例后发生，箭头指向前一个用例，如“打印订单”是“产生订单”的扩展。 |
| 包含   | ![image-20220331183456451](.Software-Engineering-assets/image-20220331183456451.png) | 虚线箭头附加构造型，表示一个用例是另一个用例的一部分，箭头指向父用例，如“验证”包含“登录”和“注册”。 |
...
**有限状态机（FSM）**是一种描述动态系统的方法，一个 FSM 由五个部分组成：
...
每个**当前状态**都在特点特定**事件**发生后，经过或不经过特定**谓词**处理，转换至**下一个状态**，或转换为**最终状态**不再变化。
